...
- **Listening practice** — two modes. *Vocabulary*: your browser's built-in Japanese text-to-speech speaks a word, pick the meaning you heard. *Conversations*: short dialogues (hand-authored, 60 total across N5/N4, each with 2 possible comprehension questions picked at random per session) read aloud in rounds of 8, then a task/point-comprehension question — closer to the real listening section's format than isolated-word recognition, with a live "who's speaking" indicator during playback (per-speaker pitch variation) and a speaker-labeled transcript revealed after answering. No audio files, no API keys — just whatever Japanese voice your OS/browser already has; the whole Listening section hides itself if none is available.
...
- `src/data/sentenceOrder.js` (sentence order) and `src/data/listeningConversations.js` (conversation listening) **are** separately hand-authored — 20 sentences and 60 conversations (2 comprehension questions each, 120 total) respectively — because no dataset of correctly-chunked JLPT-style sentences or task-based dialogue scripts exists to source from. Each was individually checked for grammaticality before inclusion, same standard as the grammar corrections above.
